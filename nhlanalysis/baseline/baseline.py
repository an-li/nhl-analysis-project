...
def baseline_models(df_train: pd.DataFrame, project_name: str, workspace: str, comet: bool = True,
    ...
    for i in model_list:
        ...
        models["LogisticRegression_" + "_".join(i)] = {"model": clf, "val_preds": val_preds, "score_prob": score_prob,
                                                       "f1": f1}

        if comet:
            experiment.log_model("LogisticRegression_" + "_".join(i),
                                 "./models/LogisticRegression_" + "_".join(i) + ".pkl")
