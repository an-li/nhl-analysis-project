...
def _extract_players_by_type(plays_df: pd.DataFrame, play_type: str) -> pd.DataFrame:
    """
    Extract players into columns by player type and play type and add them to the data frame representing live, play-by-play data

    Args:
        plays_df: Data frame containing all play-by-play data
        play_type: Type of play to extract players from

    Returns:
        Data frame with additional columns for each type of player implied in the play

    """
    plays_for_type = plays_df[plays_df['event'] == play_type]
